...
enum class http_verb { del, get, put, post };
...
static std::map<std::string, std::string> content_types{
        ...
        {".htm", "text/html"}, {".json", "text/json"},
        {".xml", "text/xml"},  {".txt", "text/txt"},
...
class property_readonly {
  ...
  public:
    ...
    const typename T::mapped_type operator[](typename T::key_type key) const {
        return value.at(key);
    }
};
