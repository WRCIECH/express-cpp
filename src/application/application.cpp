...
void application::register_route(const http_verb verb, const routePath route,
                                 const routeHandler rHandler) {
    actions _actions;
    handler _handler;
...
void application::listen(int port, std::string address) {
    ...
    server.default_resource["POST"] = [&](std::shared_ptr<HttpServer::Response> res,
                                          std::shared_ptr<HttpServer::Request> req) {
        connect_route(http_verb::post, res, req);
    };

    server.default_resource["PUT"] = [&](std::shared_ptr<HttpServer::Response> res,
                                         std::shared_ptr<HttpServer::Request> req) {
        connect_route(http_verb::put, res, req);
    };

    server.default_resource["DELETE"] = [&](std::shared_ptr<HttpServer::Response> res,
                                            std::shared_ptr<HttpServer::Request> req) {
...
void application::default_(const default_file def) { _default_file = def; }

void application::connect_route(const http_verb verb, std::shared_ptr<HttpServer::Response> res,
                                std::shared_ptr<HttpServer::Request> req) {
    ...
    if (!match_file(_res, req->path)) _res.sendStatus(http_status::http_not_found);
}
...
void application::extract_parameters(const routePath clientPath, const routePath serverRegx,
